...
let EscrowContract = (_dec = NearBindgen({}), _dec2 = call({
  payableFunction: true
}), _dec3 = call({
  privateFunction: true
}), _dec4 = call({}), _dec5 = call({}), _dec6 = call({}), _dec7 = view(), _dec(_class = (_class2 = class EscrowContract {
  GAS_FEE = BigInt(30_000_000_000_000); // 30 TGAS
  accountsReceivers = new LookupMap("ea");
  accountsValueLocked = new LookupMap("avl");
  accountAssetsQuantity = new LookupMap("aa");
  ...
  internalSendNEAR(receivingAccountId, amountBigInt) {
    ...
    log(`Sending ${amountBigInt} yoctoNEAR to ${receivingAccountId} in internalSendNEAR`);
  ...
  internalCompleteNEARTransaction(sellerAccountId, amountBigInt, buyerAccountId) {
    ...
    log(`Removing ${buyerAccountId} from accountsReceivers`);
  ...
  internalCrossContractTransferAsset(assetContractId, quantityBigInt, fromAccountId, toAccountId) {
    ...
    promise.functionCall("transfer_asset", JSON.stringify({
  ...
  validate_purchase_asset({
    validate,
    buyer_account_id,
    asset_contract_id
  }) {
    const callerId = predecessorAccountId();
    ...
    if (validate) {
      ...
      log(`Approved purchase of ${asset_contract_id} by ${buyer_account_id}`);
    } else {
      ...
      log(`Cancelled purchase of ${asset_contract_id} by ${buyer_account_id}`);
    }
  }
  purchase_in_escrow({
    seller_account_id,
    ...
    const buyerAccountId = predecessorAccountId();
    ...
    assert(buyerAccountId !== currentAccountId(), "Cannot escrow from the contract itself");
    ...
    this.accountsTimeCreated.set(buyerAccountId, blockTimestamp().toString());
    this.accountAssetsQuantity.set(buyerAccountId, "0");
    const promise = NearPromise.new(asset_contract_id);
    log(`Escrow purchase from ${asset_contract_id} ${buyerAccountId} to ${seller_account_id} for ${nearAmount} yoctoNEAR`);
    log(`Promise as return ${promise.asReturn()}}`);
    promise.functionCall("escrow_purchase_asset", JSON.stringify({
      seller_account_id,
      buyer_account_id: buyerAccountId,
      attached_near: nearAmount.toString()
    }), BigInt(0), this.GAS_FEE).then(NearPromise.new(currentAccountId()).functionCall("internalPurchaseEscrow", JSON.stringify({
      validation_user_request,
      buyerAccountId,
      asset_contract_id
    }), BigInt(0), this.GAS_FEE));
  ...
  internalPurchaseEscrow({
    ...
    log(`Escrow purchase result: ${promiseResult$1}`);
    ...
    if (promiseObject.success === false) {
      ...
      log(`Escrow purchase failed, returning ${amount} yoctoNEAR to: ${promiseObject.buyer_account_id}`);
  ...
  escrow_timeout_scan({}) {
    const callerId = predecessorAccountId();
    const timeout = callerId === "test.near" ? BigInt(-1) : BigInt(86_400_000_000_000); // 24 hours in nanoseconds. Testing workaround until fast-forward is implemented in workspaces-js
    const keys = this.accountsTimeCreated.toArray();
    keys.forEach(([buyerAccountId, _]) => {
      const timeCreatedStr = this.accountsTimeCreated.get(buyerAccountId);
      if (timeCreatedStr) {
        const timeCreated = BigInt(timeCreatedStr);
        if (timeCreated + timeout < blockTimestamp()) {
  ...
  approve_purchase({}) {
    ...
    const amount = BigInt(this.accountsValueLocked.get(buyerAccountId));
    log(`Approving escrow purchase of ${amount} yoctoNEAR from: ${buyerAccountId} to: ${sellerAccountId}`);
  ...
  cancel_purchase({}) {
    ...
    const quantity = BigInt(this.accountAssetsQuantity.get(buyerAccountId));
    ...
    log(`Canceling escrow purchase of ${quantity} from: ${buyerAccountId} to: ${sellerAccountId}`);
...
}, (_applyDecoratedDescriptor(_class2.prototype, "purchase_in_escrow", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "purchase_in_escrow"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "internalPurchaseEscrow", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "internalPurchaseEscrow"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "escrow_timeout_scan", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "escrow_timeout_scan"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "approve_purchase", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "approve_purchase"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "cancel_purchase", [_dec6], Object.getOwnPropertyDescriptor(_class2.prototype, "cancel_purchase"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "view_pending_escrow", [_dec7], Object.getOwnPropertyDescriptor(_class2.prototype, "view_pending_escrow"), _class2.prototype)), _class2)) || _class);
function view_pending_escrow() {
  const _state = EscrowContract._getState();
  if (!_state && EscrowContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = EscrowContract._create();
  if (_state) {
    EscrowContract._reconstruct(_contract, _state);
  }
  const _args = EscrowContract._getArgs();
  const _result = _contract.view_pending_escrow(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(EscrowContract._serialize(_result, true));
}
function cancel_purchase() {
  ...
  const _args = EscrowContract._getArgs();
  const _result = _contract.cancel_purchase(_args);
  EscrowContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(EscrowContract._serialize(_result, true));
}
function approve_purchase() {
  ...
  const _args = EscrowContract._getArgs();
  const _result = _contract.approve_purchase(_args);
  EscrowContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(EscrowContract._serialize(_result, true));
}
function escrow_timeout_scan() {
  const _state = EscrowContract._getState();
  if (!_state && EscrowContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = EscrowContract._getArgs();
  const _result = _contract.escrow_timeout_scan(_args);
  EscrowContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(EscrowContract._serialize(_result, true));
}
function internalPurchaseEscrow() {
  const _state = EscrowContract._getState();
  if (!_state && EscrowContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = EscrowContract._getArgs();
  const _result = _contract.internalPurchaseEscrow(_args);
  EscrowContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(EscrowContract._serialize(_result, true));
}
function purchase_in_escrow() {
  const _state = EscrowContract._getState();
  if (!_state && EscrowContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = EscrowContract._getArgs();
  const _result = _contract.purchase_in_escrow(_args);
  EscrowContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(EscrowContract._serialize(_result, true));
}

export { EscrowContract, approve_purchase, cancel_purchase, escrow_timeout_scan, internalPurchaseEscrow, purchase_in_escrow, view_pending_escrow };
//# sourceMappingURL=escrow_near.js.map
